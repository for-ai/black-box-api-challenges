...
def make_generations_col(
    generations: List[str], responses: List[Dict], results_from_span: bool = False
) -> Dict:
...
def collate(
    ...
    generations_col_iter = make_generations_col(
        generations, responses, results_from_span=results_from_span
    )
...
def main(
    ...
    if isinstance(generations[column_name].iloc[0], dict):
        ...
        gen_list = np.stack(
            generations[column_name].apply(lambda x: [y.get("text", "") if isinstance(y, dict) else y for y in x])
        )
    else:
    ...
    for chunk_idx, chunk in enumerate(tqdm(scores, desc="Collating chunks", position=0)):
        ...
        scored_gens = collate(
            gen_list[start:end], scores_list, prompts, indexes, results_from_span=results_from_span
        )
